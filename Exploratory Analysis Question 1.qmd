...
We can examine our model's predictive performance by first examining the below plots of predicted obesity probabilities against each explanatory variable.
...
We first note that our findings from the model output table are confirmed here. However, we also can note that our model seems hesitant to assign a probability greater than 0.5 to any observation in our data set. To expand on this, we first can check our model assumptions via the `performance` library.
