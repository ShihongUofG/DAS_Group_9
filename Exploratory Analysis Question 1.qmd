...
We see from @fig-diff-roc-plot that as a whole our model seems to be better than just randomly guessing, but we can further examine this by looking at the observations predicted to be most likely to be obese.
...
model_1_pred[order(model_1_pred$.fitted, decreasing = T),] %>%
...
These metrics reveal that our model's true negative rate of prediction is an acceptable level, it struggles to correctly predict when an observation is obese. These results show that our model struggles to differentiate obesity observations based on age, socio-economic status or lifestyle factors.
